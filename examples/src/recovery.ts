...
async function run() {
  ...
  console.log("IndidAddress guardianAddress", IndidAddress.fromSecp256k1(guardianAddress));
  ...
  const { accountAddress: accountAddress2 } = await clientUser.getCounterfactualAddress(
    ...
    salt,
    {
      guardians: [IndidAddress.fromSecp256k1(guardianAddress)]
    }
  );
  console.log("accountAddress2 from getCounterfactualAddress", accountAddress2);
  ...
  // const address = "0xFC70cEc245F36782E10e2cA57E8d513D81C6c784"
  ...
  // const address2 = "0xef519d389d3b7f9e9d25a8b3e9d4cee214916079"
  ...
  const guardianSigner = IndidSigner.fromSecp256k1(guardianPrivKey!, SignerKind.Guardian);

  // const { error: connectError } = await clientRecovery.connectAccount(guardianSigner, address)
  ...
  const recoveryOp = await clientUser.prepareEnterpriseRecoveryOperation(address, 
